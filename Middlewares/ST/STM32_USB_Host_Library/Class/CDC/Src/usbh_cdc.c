...
* @{
*/
#define USBH_CDC_BUFFER_SIZE                 256
/**
* @}
*/
...
static USBH_StatusTypeDef USBH_CDC_InterfaceInit(USBH_HandleTypeDef *phost)
{
  ...
  interface = USBH_FindInterface(phost,
                                 ...
                                 NO_CLASS_SPECIFIC_PROTOCOL_CODE);//COMMON_AT_COMMAND); //установка связи

  if ((interface == 0xFFU) || (interface >= USBH_MAX_NUM_INTERFACES)) /* No Valid Interface */
  {
    USBH_DbgLog("Cannot Find the interface for Communication Interface Class.", phost->pActiveClass->Name);
    return USBH_FAIL; //Не проходит
  }
  ...
  if (phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[0].bEndpointAddress & 0x80U) //захардкоженный адрес 0x80U
  {
  ...
  //проба пера:
  //interface = USBH_FindInterface(phost, USB_CDC_CLASS, ABSTRACT_CONTROL_MODEL, NO_CLASS_SPECIFIC_PROTOCOL_CODE);
  ...
  if ((phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[1].bEndpointAddress) & 0x80U) //захардкоженный адрес 0x80U
  {
  ...
  //Проба пера:
  /*CDC_Handle->DataItf.InEp = 1;
  CDC_Handle->DataItf.InEpSize  = 64;
  CDC_Handle->DataItf.OutEp = 0x80U;
  CDC_Handle->DataItf.OutEpSize = 64; */
...
static USBH_StatusTypeDef USBH_CDC_Process(USBH_HandleTypeDef *phost)
{
  ...
  switch (CDC_Handle->state)
  {
    ...
    case CDC_TRANSFER_DATA: //нужно попасть
...
* @retval None
*/
USBH_StatusTypeDef  USBH_CDC_Receive(USBH_HandleTypeDef *phost, uint8_t *pbuff, uint32_t length) //Функция приема USB
{
  USBH_StatusTypeDef Status = USBH_BUSY;
  CDC_HandleTypeDef *CDC_Handle = (CDC_HandleTypeDef *) phost->pActiveClass->pData;
...
static void CDC_ProcessReception(USBH_HandleTypeDef *phost)
{
  ...
  switch (CDC_Handle->data_rx_state)
  {
    ...
    case CDC_RECEIVE_DATA_WAIT: //Приём по USB
